<template>
    <div class="position-absolute top-50 start-50 translate-middle border p-5 rounded shadow">
        ...
        <h3 v-if="multiplicacao != null" class="mt-3 text-center">O produto entre {{ n1 }} e {{ n2 }} é {{ multiplicacao }}</h3>
    </div>
</template>
...
<script setup>
    ...
    function multiplicar() {
        ...
        service.multiplicar(n1.value, n2.value).then((response) => {
            multiplicacao.value = response.data
        }, (error) => {
